...
const DARK_PURPLE = 0x2e0a4f;
const DEEP_BLACK = 0x05010a;
const GOLD = 0xd4af37;
const LIGHT_GOLD = 0xf0d060;

const rand = (min: number, max: number) => Math.random() * (max - min) + min;

// ─── Human Silhouette Geometry (simplified wireframe figure) ────────────────────
function createHumanSilhouette(colorHex: number): THREE.Group {
...
const ThreeDBackgroundContent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.FogExp2(DEEP_BLACK, 0.01);
    ...
    renderer.setClearColor(DEEP_BLACK, 1); // Dark background
    renderer.setPixelRatio(Math.min(window.devicePixelRatio, 1.5));
    containerRef.current.appendChild(renderer.domElement);

    // ── Lights ──
    scene.add(new THREE.AmbientLight(DARK_PURPLE, 1.2));

    const light1 = new THREE.PointLight(PURPLE, 4, 200);
    light1.position.set(0, 60, 30);
    scene.add(light1);

    const light2 = new THREE.PointLight(GOLD, 3, 180);
    light2.position.set(-50, 20, -20);
    scene.add(light2);

    const light3 = new THREE.PointLight(VIOLET, 2.5, 150);
    ...
    // Main grid
    const gridHelper = new THREE.GridHelper(400, 60, GOLD, PURPLE);
    ...
    const gridHelper2 = new THREE.GridHelper(400, 120, VIOLET, DARK_PURPLE);
    (gridHelper2.material as THREE.Material).transparent = true;
    (gridHelper2.material as THREE.Material).opacity = 0.15;
    ...
    const colors = [PURPLE, GOLD, LIGHT_GOLD, VIOLET];

    for (let i = 0; i < SILHOUETTE_COUNT; i++) {
      const chosenColor = colors[i % colors.length];
      const human = createHumanSilhouette(chosenColor);
    ...
    for (let i = 0; i < RING_COUNT; i++) {
      ...
      const ringGeo = new THREE.TorusGeometry(radius, rand(0.05, 0.2), 8, 50);
    ...
    // ══════════════════════════════════════════════════════════════════════════
    const scanBeamGeo = new THREE.PlaneGeometry(300, 1.0);
    ...
    let scrollY = 0, tScrollY = window.scrollY, lastScrollY = window.scrollY;
    ...
    const animate = () => {
      ...
      // Calculate scroll velocity for transitions/animations
      const deltaScroll = tScrollY - lastScrollY;
      scrollVelocity += (deltaScroll - scrollVelocity) * 0.1;
      lastScrollY = tScrollY;

      // Smooth inputs
      mouseX += (tMouseX - mouseX) * 0.04;
      mouseY += (tMouseY - mouseY) * 0.04;
      scrollY += (tScrollY - scrollY) * 0.05; // Make scroll transition smoother but faster

      // Camera parallax with increased scroll effect (transition when scrolling down)
      camera.position.x += (mouseX * 12 - camera.position.x) * 0.025;
      // Drop camera slightly, and look slightly up when scrolled down
      camera.position.y += (15 + mouseY * 6 + scrollY * -0.015 - camera.position.y) * 0.025;
      camera.lookAt(0, 5 + scrollY * 0.005, 0); 
      
      // Rotate camera slightly on Z axis based on scroll to give a "flying through" feeling
      camera.rotation.z = Math.sin(scrollY * 0.001) * 0.05;

      // 1) Grid slow upward drift + speed up on scroll
      gridGroup.position.z = (t * 3 + scrollY * 0.05) % 6.67; 
      gridGroup.position.y = -10 + Math.sin(scrollY * 0.002) * 2; // Grid waves with scroll

      // 2) Silhouettes: gentle breathing + hover
      silhouettes.forEach((s) => {
        const breathe = Math.sin(t * 1.5 + s.animOffset) * 0.8;
        s.group.position.y = s.baseY + breathe + (Math.abs(scrollVelocity) * 0.02); // React to scroll
        s.group.rotation.y += scrollVelocity * 0.0005; // Spin slightly on scroll

        // Holographic flicker
        s.group.traverse((child) => {
          if (child instanceof THREE.Mesh && child.material instanceof THREE.MeshBasicMaterial) {
            child.material.opacity = 0.15 + Math.sin(t * 3 + s.animOffset) * 0.05;
          }
        });
      });
      ...
      columns.forEach((col) => {
        ...
        mat.opacity = 0.1 + Math.sin(t * 2 + col.animOffset) * 0.08;
      });

      // 4) Ascending particles + accelerate on scroll
      const posArray = particleGeo.attributes.position.array as Float32Array;
      const extraSpeed = Math.abs(scrollVelocity) * 0.03; // Accelerate particles on scroll
      
      for (let i = 0; i < PARTICLE_COUNT; i++) {
        const pd = particleData[i];
        pd.y += pd.speed + extraSpeed;
        if (pd.y > pd.maxY) {
          pd.y = pd.minY;
          pd.x = rand(-150, 150);
          pd.z = rand(-100, 40);
        }
        posArray[i * 3] = pd.x + Math.sin(t * 0.5 + i * 0.1) * 0.5;
        posArray[i * 3 + 1] = pd.y;
        posArray[i * 3 + 2] = pd.z;
      }
      ...
      holoRings.forEach((hr) => {
        hr.mesh.rotation.x += hr.rotSpeed.x + (scrollVelocity * 0.001);
        hr.mesh.rotation.y += hr.rotSpeed.y + (scrollVelocity * 0.001);
        hr.mesh.rotation.z += hr.rotSpeed.z;
        hr.mesh.position.y = hr.baseY + Math.sin(t * 0.8 + hr.bobOffset) * 2;
      });

      // 6) Scanning beam sweep
      scanBeam.position.y = -10 + ((t * 12 + scrollY * 0.1) % 100);
      scanBeamMat.opacity = 0.08 + Math.sin(t * 1.5) * 0.06;

      // 7) Stars slow drift
      stars.rotation.y += 0.0001 + (scrollVelocity * 0.00002);
      stars.position.y = scrollY * 0.01; // Stars parallax

      // Grid opacity pulse
      (gridHelper.material as THREE.Material).opacity = 0.25 + Math.sin(t * 0.5) * 0.08;
      (gridHelper2.material as THREE.Material).opacity = 0.15 + Math.sin(t * 0.7) * 0.05;
